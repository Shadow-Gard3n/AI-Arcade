...
//       setHistory(prev => {
//         // FIX: Prevent duplicate hints from rendering
//         // We check if the raw word OR the emoji-tagged word is already in the list
//         const isDuplicate = prev.some(item => 
//           item.guess === data.guess || item.guess === `💡 ${data.guess}`
//         );
        
//         if (isDuplicate) return prev; // Do nothing if it's a duplicate

//         // Otherwise, tag it and add it
...
// //       data.guess = `💡 ${data.guess}`;
// //       setHistory(prev => [...prev, data].sort((a, b) => a.rank - b.rank));
// //     } catch (err) {
// //       setError("Failed to get hint.");
// //     } finally {
// //       setLoading(false);
...
//   // 🔵 NEW DARK/BLUE THEME LOGIC
//   const getRankStyle = (rank) => {
//     if (rank === 1) return { bg: '#2563eb', text: 'white', border: '#3b82f6', bar: 'white' }; // Electric Blue
//     if (rank <= 100) return { bg: '#1e3a8a', text: '#e0e7ff', border: '#2563eb', bar: '#60a5fa' }; // Deep Blue
//     if (rank <= 1000) return { bg: '#1e293b', text: '#94a3b8', border: '#334155', bar: '#3b82f6' }; // Slate
//     return { bg: '#121212', text: '#64748b', border: '#27272a', bar: '#1e293b' }; // Dark Gray
//   };
...
//           <div style={{ backgroundColor: 'rgba(37, 99, 235, 0.1)', border: '2px solid #3b82f6', padding: '25px', borderRadius: '16px', textAlign: 'center', marginBottom: '30px', boxShadow: '0 0 20px rgba(59, 130, 246, 0.2)' }}>
//             <h2 style={{ color: '#60a5fa', margin: '0 0 10px 0' }}>🎯 Target Acquired!</h2>
//             <p style={{ color: '#e2e8f0', margin: 0 }}>Total guesses: {history.length}</p>
//           </div>
//         )}
...
//             onChange={(e) => setGuess(e.target.value)}
//             placeholder="Type your guess..."
//             disabled={loading || hasWon}
//             style={{ flex: 1, padding: '18px 25px', fontSize: '18px', borderRadius: '12px', border: '2px solid #1e293b', backgroundColor: '#0f172a', color: 'white', outline: 'none', transition: 'border 0.2s' }}
//             autoFocus
//           />
//           <button type="submit" disabled={loading || hasWon} style={{ padding: '0 35px', fontSize: '18px', backgroundColor: '#2563eb', color: 'white', border: 'none', borderRadius: '12px', cursor: 'pointer', fontWeight: 'bold', transition: 'background 0.2s' }}>
//             {loading ? '...' : 'Guess'}
//           </button>
//         </form>

//         <div style={{ display: 'flex', justifyContent: 'center', gap: '20px', marginBottom: '40px' }}>
//           <button onClick={handleHint} disabled={loading || hasWon} style={{ background: '#0f172a', color: '#60a5fa', border: '1px solid #1e3a8a', padding: '10px 24px', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold' }}>
//             💡 Hint
//           </button>
//           <button onClick={handleGiveUp} disabled={loading || hasWon} style={{ background: 'transparent', color: '#64748b', border: 'none', textDecoration: 'underline', cursor: 'pointer' }}>
//             I Give Up
//           </button>
//         </div>
